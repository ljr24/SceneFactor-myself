...
class Encoder3D(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 activ='gelu', disable_mid_attn=False,
                 **ignore_kwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
        ...
        self.in_channels = in_channels
        self.disable_mid_attn = disable_mid_attn
        ...
        self.mid.block_1 = ResnetBlock(in_channels=block_in,
                                       ...
                                       temb_channels=self.temb_ch,
                                       dropout=dropout)
        if not self.disable_mid_attn:
            self.mid.attn_1 = AttnBlock(block_in) # optional (disable for semantic model)
    ...
    def forward(self, x):
        ...
        if not self.disable_mid_attn:
            h = self.mid.attn_1(h) # optional (disable for semantic model)
...
class Decoder3D(nn.Module):
    ...
    def forward(self, z):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
            # if i_level != 0 and i_level != self.num_resolutions-1:
